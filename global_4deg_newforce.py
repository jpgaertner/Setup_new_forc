...
import versis.flux_atmOcn_atmIce_new as flux

import veros.tools
# import veros.tools.constants as ct
# import veros.tools.flux_atmOcn_atmIce_new as flux
...
class GlobalFourDegreeSetup(VerosSetup):
    ...
    def set_parameter(self, state):
        ...
        state.var_meta.update(
            ...
            ocn_lwnet=Variable("ocn_lwnet", hor_dim, "", "", time_dependent=False),
            ice_lwdw=Variable("ice_lwdw", hor_dim, "", "", time_dependent=False),
            ice_lwup=Variable("ice_lwup", hor_dim, "", "", time_dependent=False),
            ocn_sen=Variable("ocn_sen", hor_dim, "", "", time_dependent=False),
            ocn_lat=Variable("ocn_lat", hor_dim, "", "", time_dependent=False),
            ice_sen=Variable("ice_sen", hor_dim, "", "", time_dependent=False),
            ice_lat=Variable("ice_lat", hor_dim, "", "", time_dependent=False),
            ocn_taux=Variable("ocn_taux", hor_dim, "", "", time_dependent=False),
            ocn_tauy=Variable("ocn_tauy", hor_dim, "", "", time_dependent=False),
            ice_taux=Variable("ice_taux", hor_dim, "", "", time_dependent=False),
            ice_tauy=Variable("ice_tauy", hor_dim, "", "", time_dependent=False),
            maskT_noI=Variable("maskT_noI", hor_dim, "", "", time_dependent=False),
            #
    ...
    def set_topography(self, state):
        ...
        zt_forc = self._read_forcing("zt")[::-1] #ront
...
def set_forcing_kernel(state):
    ...
    ocn_sen, ocn_lat, ocn_lwup, ocn_evap, ocn_taux, ocn_tauy, ocn_tref, ocn_qref, ocn_duu10n,\
        ocn_ustar, ocn_tstar, ocn_qstar = \
        flux.flux_atmOcn(maskT_noI, rbot,
                         zbot, ubot, vbot,
                         qbot, tbot, thbot,
                         vs.u[:, :, -1, vs.tau],
                         vs.v[:, :, -1, vs.tau],
                         temp)
    # ice net surface heat flux
    ice_sen, ice_lat, ice_lwup, ice_evap, ice_taux, ice_tauy, ice_tref, ice_qref,\
        ice_ustar, ice_tstar, ice_qstar = \
        flux.flux_atmIce(maskI, rbot, zbot, ubot, vbot, qbot, tbot, thbot,
                         temp) # replace vs.temp with sea-ice temperature
    # Net LW radiation flux from sea surface
    ocn_lwnet = flux.net_lw_ocn(state, maskT_noI, vs.yt[:], qbot,
                                temp, tbot, tcc)

    # Downward LW radiation flux over sea-ice
    ice_lwdw = flux.dw_lw_ice(maskI, tbot, tcc)

    qir, qh, qe = flux.flux_atmOcnIce(vs.maskT[:, :, -1], spres, qbot, rbot, ubot, vbot, tbot,
                                      vs.u[:, :, -1, vs.tau], vs.v[:, :, -1, vs.tau],
                                      temp)

    qnet_simple_new = swr_net + qir + lwr_dw + qh + qe

    dqir_dt, dqh_dt, dqe_dt = flux.dqnetdt(vs.maskT[:, :, -1], spres, rbot, temp,
                                           ubot, vbot, vs.u[:, :, -1, vs.tau], vs.v[:, :, -1, vs.tau])
    # --------------------------------------------------------

    # wind stress
    #if use_new_forcing:
    #    vs.surface_taux = ocn_taux + ice_taux 
    #    vs.surface_tauy = ocn_tauy + ice_tauy
    #else:
